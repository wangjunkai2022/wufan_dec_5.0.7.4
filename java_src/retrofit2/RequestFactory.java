    ...
    public static final class Builder {
        ...
        private void parseMethodAnnotation(Annotation annotation) {
            if (annotation instanceof DELETE) {
                parseHttpMethodAndPath(HttpDeleteHC4.METHOD_NAME, ((DELETE) annotation).value(), false);
            } else if (annotation instanceof GET) {
                parseHttpMethodAndPath("GET", ((GET) annotation).value(), false);
            } else if (annotation instanceof HEAD) {
                parseHttpMethodAndPath(HttpHeadHC4.METHOD_NAME, ((HEAD) annotation).value(), false);
            } else if (annotation instanceof PATCH) {
                parseHttpMethodAndPath(HttpPatch.METHOD_NAME, ((PATCH) annotation).value(), true);
            } else if (annotation instanceof POST) {
                parseHttpMethodAndPath("POST", ((POST) annotation).value(), true);
            } else if (annotation instanceof PUT) {
                parseHttpMethodAndPath(HttpPutHC4.METHOD_NAME, ((PUT) annotation).value(), true);
            } else if (annotation instanceof OPTIONS) {
                parseHttpMethodAndPath(HttpOptionsHC4.METHOD_NAME, ((OPTIONS) annotation).value(), false);
        ...
        @Nullable
        private ParameterHandler<?> parseParameter(int i4, Type type, @Nullable Annotation[] annotationArr, boolean z4) {
        ...
        private ParameterHandler<?> parseParameterAnnotation(int i4, Type type, Annotation[] annotationArr, Annotation annotation) {
            if (annotation instanceof Url) {
                ...
                if (this.isMultipart) {
                    ...
                    Headers of = Headers.of("Content-Disposition", "form-data; name=\"" + value5 + m.a.f71493g, HttpHeaders.Names.CONTENT_TRANSFER_ENCODING, part.encoding());
                    if (Iterable.class.isAssignableFrom(rawType8)) {
                        if (type instanceof ParameterizedType) {
                ...
                if (!this.isFormEncoded && !this.isMultipart) {
                    if (!this.gotBody) {
                        try {
                            ...
                            throw Utils.parameterError(this.method, e5, i4, "Unable to create @Body converter for %s", type);
                        }
                    }
        ...
        RequestFactory build() {
            ...
            if (this.httpMethod != null) {
                ...
                while (true) {
                    ...
                    parameterHandlerArr[i5] = parseParameter(i5, type, annotationArr, z4);
                    i5++;
                }
