package com.fasterxml.jackson.databind.introspect;

import com.fasterxml.jackson.databind.JavaType;
import java.lang.annotation.Annotation;
import java.lang.reflect.Type;
/* loaded from: classes3.dex */
public abstract class AnnotatedWithParams extends AnnotatedMember {
    private static final long serialVersionUID = 1;
    protected final AnnotationMap[] _paramAnnotations;

    /* JADX INFO: Access modifiers changed from: protected */
    public AnnotatedWithParams(TypeResolutionContext typeResolutionContext, AnnotationMap annotationMap, AnnotationMap[] annotationMapArr) {
        super(typeResolutionContext, annotationMap);
        this._paramAnnotations = annotationMapArr;
    }

    public final void addOrOverrideParam(int i4, Annotation annotation) {
        AnnotationMap annotationMap = this._paramAnnotations[i4];
        if (annotationMap == null) {
            annotationMap = new AnnotationMap();
            this._paramAnnotations[i4] = annotationMap;
        }
        annotationMap.add(annotation);
    }

    public abstract Object call() throws Exception;

    public abstract Object call(Object[] objArr) throws Exception;

    public abstract Object call1(Object obj) throws Exception;

    public final int getAnnotationCount() {
        return this._annotations.size();
    }

    @Deprecated
    public abstract Type getGenericParameterType(int i4);

    public final AnnotatedParameter getParameter(int i4) {
        return new AnnotatedParameter(this, getParameterType(i4), this._typeContext, getParameterAnnotations(i4), i4);
    }

    public final AnnotationMap getParameterAnnotations(int i4) {
        AnnotationMap[] annotationMapArr = this._paramAnnotations;
        if (annotationMapArr == null || i4 < 0 || i4 >= annotationMapArr.length) {
            return null;
        }
        return annotationMapArr[i4];
    }

    public abstract int getParameterCount();

    public abstract JavaType getParameterType(int i4);

    public abstract Class<?> getRawParameterType(int i4);

    /* JADX INFO: Access modifiers changed from: protected */
    public AnnotatedParameter replaceParameterAnnotations(int i4, AnnotationMap annotationMap) {
        this._paramAnnotations[i4] = annotationMap;
        return getParameter(i4);
    }

    protected AnnotatedWithParams(AnnotatedWithParams annotatedWithParams, AnnotationMap[] annotationMapArr) {
        super(annotatedWithParams);
        this._paramAnnotations = annotationMapArr;
    }
}
