...
public class MappingIterator<T> implements Iterator<T>, Closeable {
    ...
    public MappingIterator(JavaType javaType, JsonParser jsonParser, DeserializationContext deserializationContext, JsonDeserializer<?> jsonDeserializer, boolean z4, Object obj) {
        this._type = javaType;
        this._parser = jsonParser;
        this._context = deserializationContext;
        this._deserializer = jsonDeserializer;
        this._closeParser = z4;
        if (obj == 0) {
        ...
        if (z4 && jsonParser.isExpectedStartArrayToken()) {
    ...
    public boolean hasNext() {
        try {
            ...
            return ((Boolean) _handleMappingException(e5)).booleanValue();
        } catch (IOException e6) {
            return ((Boolean) _handleIOException(e6)).booleanValue();
        }
    }
    ...
    public T next() {
        try {
            ...
            return (T) _handleMappingException(e5);
        } catch (IOException e6) {
            return (T) _handleIOException(e6);
        }
    }
    ...
    public T nextValue() throws IOException {
        ...
        if (i4 != 0) {
            ...
            try {
                T t5 = this._updatedValue;
                if (t5 == null) {
                    t4 = this._deserializer.deserialize(this._parser, this._context);
                } else {
                    this._deserializer.deserialize(this._parser, this._context, t5);
                    t4 = this._updatedValue;
                }
                this._state = 2;
    ...
    public <L extends List<? super T>> L readAll(L l4) throws IOException {
        while (hasNextValue()) {
            l4.add(nextValue());
        }
        return l4;
    ...
    public <C extends Collection<? super T>> C readAll(C c5) throws IOException {
        while (hasNextValue()) {
            c5.add(nextValue());
        }
        return c5;
