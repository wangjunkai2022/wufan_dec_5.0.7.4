...
public abstract class TokenStreamFactory implements Versioned, Serializable {
    ...
    public InputStream _optimizedStreamFromURL(URL url) throws IOException {
        String host;
        if (f.f10925c.equals(url.getProtocol()) && (((host = url.getHost()) == null || host.length() == 0) && url.getPath().indexOf(37) < 0)) {
    ...
    public abstract JsonParser createParser(byte[] bArr, int i4, int i5) throws IOException;

    public abstract JsonParser createParser(char[] cArr) throws IOException;

    public abstract JsonParser createParser(char[] cArr, int i4, int i5) throws IOException;
