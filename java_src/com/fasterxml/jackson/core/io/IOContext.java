...
public class IOContext {
    ...
    public char[] allocNameCopyBuffer(int i4) {
        ...
        char[] allocCharBuffer = this._bufferRecycler.allocCharBuffer(3, i4);
    ...
    public byte[] allocBase64Buffer(int i4) {
        _verifyAlloc(this._base64Buffer);
        byte[] allocByteBuffer = this._bufferRecycler.allocByteBuffer(3, i4);
        this._base64Buffer = allocByteBuffer;
        return allocByteBuffer;
    }

    public byte[] allocReadIOBuffer(int i4) {
        _verifyAlloc(this._readIOBuffer);
        byte[] allocByteBuffer = this._bufferRecycler.allocByteBuffer(0, i4);
        this._readIOBuffer = allocByteBuffer;
        return allocByteBuffer;
    }

    public char[] allocTokenBuffer(int i4) {
        _verifyAlloc(this._tokenCBuffer);
        char[] allocCharBuffer = this._bufferRecycler.allocCharBuffer(0, i4);
        this._tokenCBuffer = allocCharBuffer;
        return allocCharBuffer;
    }

    public byte[] allocWriteEncodingBuffer(int i4) {
        _verifyAlloc(this._writeEncodingBuffer);
        byte[] allocByteBuffer = this._bufferRecycler.allocByteBuffer(1, i4);
        this._writeEncodingBuffer = allocByteBuffer;
        return allocByteBuffer;
    }
}
