    ...
    private static OrmLiteSqliteOpenHelper constructHelper(Context context, Class<? extends OrmLiteSqliteOpenHelper> cls) {
        try {
            try {
                ...
                throw new IllegalStateException("Could not construct instance of helper class " + cls, e5);
            }
        } catch (Exception e6) {
            throw new IllegalStateException("Could not find public constructor that has a single (Context) argument for helper class " + cls, e6);
        }
    }
    ...
    private static Class<? extends OrmLiteSqliteOpenHelper> lookupHelperClass(Context context, Class<?> cls) {
        ...
        if (identifier != 0) {
            ...
            try {
                ...
                throw new IllegalStateException("Could not create helper instance for class " + string, e5);
            }
        }
    ...
    public static synchronized void releaseHelper() {
        synchronized (OpenHelperManager.class) {
            ...
            if (instanceCount <= 0) {
                ...
                int i4 = instanceCount;
                if (i4 < 0) {
                    logger.error("too many calls to release helper, instance count = {}", Integer.valueOf(i4));
                }
            }
        }
    }
