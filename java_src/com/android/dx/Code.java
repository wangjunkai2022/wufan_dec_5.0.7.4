...
public final class Code {
    ...
    public <T> void loadConstant(Local<T> local, T t4) {
        Rop opConst;
        if (t4 == null) {
        ...
        if (rop.getBranchingness() == 1) {
            addInstruction(new PlainCstInsn(rop, this.sourcePosition, local.spec(), RegisterSpecList.EMPTY, Constants.getConstant(t4)));
            return;
        }
        addInstruction(new ThrowingCstInsn(rop, this.sourcePosition, RegisterSpecList.EMPTY, this.catches, Constants.getConstant(t4)));
        moveResult(local, true);
    }
