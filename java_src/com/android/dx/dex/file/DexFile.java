...
public final class DexFile {
    ...
    public static final class Storage {
        ...
        public byte[] getStorage(int i4) {
            if (this.storage.length < i4) {
                Logger anonymousLogger = Logger.getAnonymousLogger();
                Level level = Level.FINER;
                anonymousLogger.log(level, "DexFile storage too small  " + this.storage.length + " vs " + i4);
                this.storage = new byte[i4];
            }
            return this.storage;
        }
    }
    ...
    private static void calcChecksum(byte[] bArr, int i4) {
    ...
    private static void calcSignature(byte[] bArr, int i4) {
        try {
    ...
    private ByteArrayAnnotatedOutput toDex0(boolean z4, boolean z5, Storage storage) {
            ...
            if ((section != this.callSiteIds && section != this.methodHandles) || !section.items().isEmpty()) {
                ...
                if (fileOffset >= i4) {
                    try {
                        ...
                        throw ExceptionWithContext.withContext(e5, "...while writing section " + i5);
                    }
                } else {
        ...
        this.fileSize = i4;
        byte[] storage2 = storage == null ? new byte[i4] : storage.getStorage(i4);
