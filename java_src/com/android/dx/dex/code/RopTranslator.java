...
public final class RopTranslator {
    ...
    private RopTranslator(RopMethod ropMethod, int i4, LocalVariableInfo localVariableInfo, int i5, DexOptions dexOptions) {
        ...
        this.paramSize = i5;
        boolean calculateParamsAreInOrder = calculateParamsAreInOrder(ropMethod, i5);
        this.paramsAreInOrder = calculateParamsAreInOrder;
        BasicBlockList blocks = ropMethod.getBlocks();
        int size = blocks.size();
        int i6 = size * 3;
        int instructionCount = blocks.getInstructionCount() + i6;
        int assignmentCount = localVariableInfo != null ? instructionCount + size + localVariableInfo.getAssignmentCount() : instructionCount;
        int regCount = blocks.getRegCount() + (calculateParamsAreInOrder ? 0 : i5);
        this.regCount = regCount;
        OutputCollector outputCollector = new OutputCollector(dexOptions, assignmentCount, i6, regCount, i5);
        this.output = outputCollector;
        ...
        ropMethod.getBlocks().forEachInsn(new Insn.BaseVisitor() { // from class: com.android.dx.dex.code.RopTranslator.1
            ...
            public void visitPlainCstInsn(PlainCstInsn plainCstInsn) {
                if (plainCstInsn.getOpcode().getOpcode() == 3) {
                    ...
                    boolean[] zArr2 = zArr;
                    zArr2[0] = zArr2[0] && (regCount - i4) + value == plainCstInsn.getResult().getReg();
                }
            }
        });
        return zArr[0];
    ...
    private void outputInstructions() {
        ...
        while (i4 < length) {
            int i5 = i4 + 1;
            outputBlock(blocks.labelToBlock(iArr[i4]), i5 == iArr.length ? -1 : iArr[i5]);
            i4 = i5;
        }
    }
