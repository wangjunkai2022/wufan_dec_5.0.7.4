...
public final class OutputFinisher {
    ...
    private static void addConstants(HashSet<Constant> hashSet, DalvInsn dalvInsn) {
        ...
        if (dalvInsn instanceof MultiCstInsn) {
            ...
            while (i4 < multiCstInsn.getNumberOfConstants()) {
                hashSet.add(multiCstInsn.getConstant(i4));
                i4++;
            }
            ...
            while (it2.hasNext()) {
                ...
                for (int i9 = 0; i9 < registers.size(); i9++) {
                    ...
                    if (registerSpec.isCategory2()) {
                        boolean z4 = registerSpec.getReg() >= i4;
    ...
    private int calculateReservedCount(Dop[] dopArr) {
        ...
        while (i4 < size) {
            ...
            if (findOpcodeForInsn == null) {
                ...
                i4 = dop == findOpcodeForInsn ? i4 + 1 : 0;
            }
            dopArr[i4] = findOpcodeForInsn;
        }
        return i5;
    }
    ...
    private static void assignIndices(MultiCstInsn multiCstInsn, DalvCode.AssignIndicesCallback assignIndicesCallback) {
        for (int i4 = 0; i4 < multiCstInsn.getNumberOfConstants(); i4++) {
            Constant constant = multiCstInsn.getConstant(i4);
            multiCstInsn.setIndex(i4, assignIndicesCallback.getIndex(constant));
