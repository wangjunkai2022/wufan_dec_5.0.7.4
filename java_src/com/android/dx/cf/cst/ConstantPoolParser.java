...
public final class ConstantPoolParser {
                ...
                switch (unsignedByte) {
                    ...
                    case 15:
                        try {
                            ...
                            e5.addContext("...while preparsing cst " + Hex.u2(i6) + " at offset " + Hex.u4(i5));
                            throw e5;
                        }
                }
                i4 = 1;
    ...
    private static int getMethodHandleTypeForKind(int i4) {
        switch (i4) {
            case 1:
                return 3;
            case 2:
            ...
            case 9:
                return 8;
            default:
                throw new IllegalArgumentException("invalid kind: " + i4);
        }
    }
        ...
        try {
            ...
            switch (unsignedByte) {
                ...
                default:
                    throw new ParseException("unknown tag byte: " + Hex.u1(unsignedByte));
                case 3:
                    make = CstInteger.make(this.bytes.getInt(i5 + 1));
                    break;
                case 4:
                    make = CstFloat.make(this.bytes.getInt(i5 + 1));
                    break;
                case 5:
                    make = CstLong.make(this.bytes.getLong(i5 + 1));
                    break;
                case 6:
                    make = CstDouble.make(this.bytes.getLong(i5 + 1));
                    break;
                case 7:
                    ...
                    break;
                case 8:
                    make = parse0(this.bytes.getUnsignedShort(i5 + 1), bitSet);
                    break;
                case 9:
                    cstFieldRef = new CstFieldRef((CstType) parse0(this.bytes.getUnsignedShort(i5 + 1), bitSet), (CstNat) parse0(this.bytes.getUnsignedShort(i5 + 3), bitSet));
                    make = cstFieldRef;
                    break;
                case 10:
                    cstFieldRef = new CstMethodRef((CstType) parse0(this.bytes.getUnsignedShort(i5 + 1), bitSet), (CstNat) parse0(this.bytes.getUnsignedShort(i5 + 3), bitSet));
                    make = cstFieldRef;
                    break;
                case 11:
                    cstFieldRef = new CstInterfaceMethodRef((CstType) parse0(this.bytes.getUnsignedShort(i5 + 1), bitSet), (CstNat) parse0(this.bytes.getUnsignedShort(i5 + 3), bitSet));
                    make = cstFieldRef;
                    break;
                case 12:
                    cstFieldRef = new CstNat((CstString) parse0(this.bytes.getUnsignedShort(i5 + 1), bitSet), (CstString) parse0(this.bytes.getUnsignedShort(i5 + 3), bitSet));
                    make = cstFieldRef;
                    break;
                case 15:
                    int unsignedByte2 = this.bytes.getUnsignedByte(i5 + 1);
                    ...
                    break;
                case 16:
                    make = CstProtoRef.make((CstString) parse0(this.bytes.getUnsignedShort(i5 + 1), bitSet));
                    break;
                case 18:
                    make = CstInvokeDynamic.make(this.bytes.getUnsignedShort(i5 + 1), (CstNat) parse0(this.bytes.getUnsignedShort(i5 + 3), bitSet));
                    break;
            }
            this.pool.set(i4, make);
            return make;
        } catch (ParseException e5) {
            e5.addContext("...while parsing cst " + Hex.u2(i4) + " at offset " + Hex.u4(i5));
            throw e5;
