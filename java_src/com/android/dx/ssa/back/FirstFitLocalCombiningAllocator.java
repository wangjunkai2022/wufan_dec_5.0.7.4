...
public class FirstFitLocalCombiningAllocator extends RegisterAllocator {
    ...
    private boolean canMapReg(RegisterSpec registerSpec, int i4) {
        return (spansParamRange(i4, registerSpec.getCategory()) || this.mapper.interferes(registerSpec, i4)) ? false : true;
    }
        ...
        while (it2.hasNext()) {
            ...
            if (predecessors.cardinality() == 1) {
                ...
                if (ssaInsn.getOpcode().getOpcode() == 43) {
                    ...
                    if (oldToNew != this.mapper.oldToNew(reg2) && !z6) {
        ...
        for (ArrayList<RegisterSpec> arrayList : this.localVariables.values()) {
            ...
            do {
                ...
                if (canMapRegs(arrayList, findRopRegForLocal)) {
                    z4 = tryMapRegs(arrayList, findRopRegForLocal, i5, true);
                }
    ...
    private void handleLocalAssociatedParams() {
        for (ArrayList<RegisterSpec> arrayList : this.localVariables.values()) {
            ...
            if (i4 >= 0) {
                tryMapRegs(arrayList, i4, i5, true);
            }
        }
    }
    ...
    private boolean isThisPointerReg(int i4) {
        return i4 == 0 && !this.ssaMeth.isStatic();
    }
    ...
    private boolean tryMapRegs(ArrayList<RegisterSpec> arrayList, int i4, int i5, boolean z4) {
        Iterator<RegisterSpec> it2 = arrayList.iterator();
        boolean z5 = false;
        while (it2.hasNext()) {
            RegisterSpec next = it2.next();
    ...
    RegisterSpecList ssaSetToSpecs(IntSet intSet) {
        ...
        int i4 = 0;
        while (it2.hasNext()) {
            registerSpecList.set(i4, getDefinitionSpecForSsaReg(it2.next()));
            i4++;
        }
