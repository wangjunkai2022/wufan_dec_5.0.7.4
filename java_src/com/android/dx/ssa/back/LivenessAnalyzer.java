...
public class LivenessAnalyzer {
    ...
    private static void coInterferePhiRegisters(InterferenceGraph interferenceGraph, RegisterSpec registerSpec, RegisterSpecList registerSpecList) {
        ...
        for (int i4 = 0; i4 < registerSpecList.size(); i4++) {
            interferenceGraph.add(reg, registerSpecList.get(i4).getReg());
        }
    }
        ...
        while (it2.hasNext()) {
            ...
            for (int i4 = 0; i4 < size; i4++) {
                for (int i5 = 0; i5 < size; i5++) {
                    if (i4 != i5) {
                        SsaInsn ssaInsn = phiInsns.get(i4);
                        SsaInsn ssaInsn2 = phiInsns.get(i5);
