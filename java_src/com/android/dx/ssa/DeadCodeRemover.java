...
public class DeadCodeRemover {
    ...
    */
    ...
    private void pruneDeadInstructions() {
        ...
        int i4 = 0;
        while (true) {
            int nextClearBit = computeReachability.nextClearBit(i4);
            if (nextClearBit < blocks.size()) {
                SsaBasicBlock ssaBasicBlock = blocks.get(nextClearBit);
                i4 = nextClearBit + 1;
                for (int i5 = 0; i5 < ssaBasicBlock.getInsns().size(); i5++) {
                    SsaInsn ssaInsn = ssaBasicBlock.getInsns().get(i5);
                    ...
                    for (int i6 = 0; i6 < size; i6++) {
                        this.useList[sources.get(i6).getReg()].remove(ssaInsn);
                    }
