...
public class EscapeAnalysis {
    ...
    private int findSetIndex(RegisterSpec registerSpec) {
        int i4 = 0;
        while (i4 < this.latticeValues.size() && !this.latticeValues.get(i4).regSet.get(registerSpec.getReg())) {
            i4++;
        }
        return i4;
    }
        ...
        for (int i4 = 0; i4 < this.ssaMeth.getRegCount(); i4++) {
            ...
            if (definitionForRegister != null && definitionForRegister.getOpcode() != null && definitionForRegister.getOpcode().getOpcode() == 2) {
                ...
                if (registerSpec.getReg() >= this.regCount || result.getReg() >= this.regCount) {
                    ...
                    Iterator<SsaInsn> it2 = useListCopy[result.getReg()].iterator();
                    while (it2.hasNext()) {
                        it2.next().mapSourceRegisters(registerMapper);
                    }
                }
            }
        }
    ...
    private void replaceNode(EscapeSet escapeSet, EscapeSet escapeSet2) {
        Iterator<EscapeSet> it2 = escapeSet2.parentSets.iterator();
        while (it2.hasNext()) {
            EscapeSet next = it2.next();
            next.childSets.remove(escapeSet2);
            next.childSets.add(escapeSet);
            escapeSet.parentSets.add(next);
        }
        Iterator<EscapeSet> it3 = escapeSet2.childSets.iterator();
        while (it3.hasNext()) {
            EscapeSet next2 = it3.next();
    ...
    private void run() {
        ...
        Iterator<EscapeSet> it2 = this.latticeValues.iterator();
        while (it2.hasNext()) {
            EscapeSet next = it2.next();
            if (next.escape != EscapeState.NONE) {
                Iterator<EscapeSet> it3 = next.childSets.iterator();
                while (it3.hasNext()) {
                    EscapeSet next2 = it3.next();
