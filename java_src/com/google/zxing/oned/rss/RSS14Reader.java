...
public final class RSS14Reader extends AbstractRSSReader {
    ...
    */
    ...
    private DataCharacter decodeDataCharacter(BitArray bitArray, FinderPattern finderPattern, boolean z4) throws NotFoundException {
        ...
        int i6 = z4 ? 16 : 15;
        float count = AbstractRSSReader.count(dataCharacterCounters) / i6;
        int[] oddCounts = getOddCounts();
        int[] evenCounts = getEvenCounts();
        float[] oddRoundingErrors = getOddRoundingErrors();
        float[] evenRoundingErrors = getEvenRoundingErrors();
        for (int i7 = 0; i7 < dataCharacterCounters.length; i7++) {
            float f5 = dataCharacterCounters[i7] / count;
            int i8 = (int) (0.5f + f5);
            if (i8 < 1) {
                i8 = 1;
            } else if (i8 > 8) {
    ...
    private Pair decodePair(BitArray bitArray, boolean z4, int i4, Map<DecodeHintType, ?> map) {
        try {
            int[] findFinderPattern = findFinderPattern(bitArray, 0, z4);
            FinderPattern parseFoundFinderPattern = parseFoundFinderPattern(bitArray, i4, z4, findFinderPattern);
            ResultPointCallback resultPointCallback = map == null ? null : (ResultPointCallback) map.get(DecodeHintType.NEED_RESULT_POINT_CALLBACK);
            if (resultPointCallback != null) {
                float f5 = (findFinderPattern[0] + findFinderPattern[1]) / 2.0f;
                if (z4) {
                    f5 = (bitArray.getSize() - 1) - f5;
                }
                resultPointCallback.foundPossibleResultPoint(new ResultPoint(f5, i4));
            }
    ...
    private int[] findFinderPattern(BitArray bitArray, int i4, boolean z4) throws NotFoundException {
    ...
    private FinderPattern parseFoundFinderPattern(BitArray bitArray, int i4, boolean z4, int[] iArr) throws NotFoundException {
        int i5;
        int i6;
        boolean z5 = bitArray.get(iArr[0]);
        int i7 = iArr[0] - 1;
        while (i7 >= 0 && (bitArray.get(i7) ^ z5)) {
            i7--;
        }
        int i8 = i7 + 1;
        int[] decodeFinderCounters = getDecodeFinderCounters();
