...
public class MonthAdapter extends BaseAdapter {
    ...
    private boolean isSelected(long j4) {
        ...
        while (it2.hasNext()) {
            if (UtcDates.canonicalYearMonthDay(j4) == UtcDates.canonicalYearMonthDay(it2.next().longValue())) {
                return true;
            }
        }
        return false;
    ...
    private void updateSelectedState(@Nullable TextView textView, long j4) {
        ...
        if (this.calendarConstraints.getDateValidator().isValid(j4)) {
            ...
            } else if (UtcDates.getTodayCalendar().getTimeInMillis() == j4) {
    ...
    private void updateSelectedStateForDate(MaterialCalendarGridView materialCalendarGridView, long j4) {
        if (Month.create(j4).equals(this.month)) {
            updateSelectedState((TextView) materialCalendarGridView.getChildAt(materialCalendarGridView.getAdapter2().dayToPosition(this.month.getDayOfMonth(j4)) - materialCalendarGridView.getFirstVisiblePosition()), j4);
        }
    }
    ...
    public void updateSelectedStates(MaterialCalendarGridView materialCalendarGridView) {
        for (Long l4 : this.previouslySelectedDates) {
            updateSelectedStateForDate(materialCalendarGridView, l4.longValue());
        }
