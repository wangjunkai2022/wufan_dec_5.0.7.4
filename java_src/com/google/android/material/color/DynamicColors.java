...
public class DynamicColors {
    ...
    public static Context wrapContextIfAvailable(@NonNull Context context, @StyleRes int i4) {
        if (isDynamicColorAvailable()) {
            if (i4 == 0) {
                i4 = getDefaultThemeOverlay(context);
            }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static void applyToActivityIfAvailable(@NonNull Activity activity, @StyleRes int i4, @NonNull Precondition precondition, @NonNull OnAppliedCallback onAppliedCallback) {
        if (isDynamicColorAvailable()) {
            if (i4 == 0) {
                i4 = getDefaultThemeOverlay(activity);
            }
