...
public class UMModuleRegister {
    ...
    public static String eventType2ModuleName(int i4) {
        String str = (i4 < PUSH_EVENT_VALUE_LOW || i4 > PUSH_EVENT_VALUE_HIGH) ? "analytics" : "push";
        if (i4 >= SHARE_EVENT_VALUE_LOW && i4 <= SHARE_EVENT_VALUE_HIGH) {
            str = "share";
        }
        if (i4 >= 32769 && i4 <= INNER_EVENT_VALUE_HIGH) {
            str = INNER;
        }
        if (i4 >= 36945 && i4 <= PROCESS_EVENT_VALUE_HIGH) {
            str = PROCESS;
        }
        return (i4 < APPSTATUS_SWITCH_LOW || i4 > APPSTATUS_SWITCH_HIGH) ? str : APPSTATUS;
    }
