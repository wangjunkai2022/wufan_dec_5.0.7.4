...
public class UMFrUtils {
    ...
    private static String mDefaultEnvelopeDir = aw.b().b(aw.f63370a);
        ...
        synchronized (mEnvelopeFileLock) {
            try {
                if (mDefaultEnvelopeDirPath == null) {
                    mDefaultEnvelopeDirPath = context.getFilesDir().getAbsolutePath() + File.separator + h0.f27805a + mDefaultEnvelopeDir;
                }
    ...
    public static void syncLegacyEnvelopeIfNeeded(Context context) {
        ...
        try {
            ...
            File file = new File(context.getFilesDir().getAbsolutePath() + bi.f55317j + legacyEnvelopeDir);
            if (file.exists()) {
                File[] listFiles = file.listFiles();
                if (listFiles != null && listFiles.length != 0) {
                    String envelopeDirPath = getEnvelopeDirPath(context);
