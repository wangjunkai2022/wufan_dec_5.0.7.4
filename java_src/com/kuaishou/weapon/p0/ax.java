...
public class ax {
    ...
    public ax(Context context) {
        if (Engine.loadSuccess) {
            try {
                String pqr = Engine.getInstance(context).pqr(Integer.valueOf(ck.f55414g).intValue(), 1, 0, "");
                if (TextUtils.isEmpty(pqr)) {
                    return;
                }
                this.f55288a = new JSONObject(pqr);
    ...
    public String a(String str) {
        JSONObject jSONObject = this.f55288a;
        if (jSONObject != null) {
            try {
            ...
            try {
                ...
                for (int i5 = 0; i5 < jSONArray.length(); i5++) {
                    ...
                    if (!string2.contains("azeroth") && !string2.contains("OkHttp") && !string2.contains("retrofit")) {
                        ...
                        if (!replace.contains(packageName) && !packageName.contains(replace) && !a5.contains(substring) && !b(i4).contains(substring)) {
                            hashSet.add(replace);
                        }
                    }
                }
