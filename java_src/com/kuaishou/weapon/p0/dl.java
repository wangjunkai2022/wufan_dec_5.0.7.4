...
public class dl {
    public static String a(String str) {
        if (str.contains(com.facebook.common.util.f.f10923a)) {
            str = str.replace("https://", "").replace("http://", "");
        }
        return str.contains(net.lingala.zip4j.util.e.F0) ? str.substring(0, str.indexOf(net.lingala.zip4j.util.e.F0)) : str;
    }
    ...
    public static Set b(Context context) {
        try {
            ...
            for (InputMethodInfo inputMethodInfo : inputMethodList) {
                hashSet.add(inputMethodInfo.getId().split(net.lingala.zip4j.util.e.F0)[0]);
            }
            if (hashSet.size() <= 0) {
                return null;
            }
