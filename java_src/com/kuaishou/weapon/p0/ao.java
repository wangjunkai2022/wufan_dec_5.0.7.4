...
public class ao {
    public boolean a() {
        try {
            Field declaredField = ClassLoader.getSystemClassLoader().loadClass(an.f55275b).getDeclaredField("disableHooks");
    ...
    public boolean b() {
        try {
            ...
            for (StackTraceElement stackTraceElement : e5.getStackTrace()) {
                if (stackTraceElement.getClassName().contains(an.f55275b)) {
                    return true;
                }
            }
            return false;
    ...
    public boolean d() {
        ...
        if (systemClassLoader != null) {
            try {
                systemClassLoader.loadClass(an.f55274a).newInstance();
        ...
        if (systemClassLoader != null) {
            try {
                systemClassLoader.loadClass(an.f55275b).newInstance();
