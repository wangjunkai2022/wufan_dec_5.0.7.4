...
public class FrameSequenceDrawable extends Drawable implements Animatable, Runnable {
    ...
    private static Bitmap acquireAndValidateBitmap(a aVar, int i4, int i5) {
        Bitmap y2 = aVar.y(i4, i5);
        if (y2.getWidth() < i4 || y2.getHeight() < i5 || y2.getConfig() != Bitmap.Config.ARGB_8888) {
    ...
    public void draw(Canvas canvas) {
        synchronized (this.mLock) {
            ...
            if (this.mCircleMaskEnabled) {
                ...
                float width = (bounds.width() * 1.0f) / f5;
                ...
                float f6 = min / width;
                float f7 = min / height;
                this.mTempRectF.set((f5 - f6) / 2.0f, (intrinsicHeight - f7) / 2.0f, (f5 + f6) / 2.0f, (intrinsicHeight + f7) / 2.0f);
    ...
    public boolean isRunning() {
        boolean z4;
        synchronized (this.mLock) {
            z4 = this.mNextFrameToDecode >= 0 && !this.mDestroyed;
        }
        return z4;
    }
    ...
    public final void setCircleMaskEnabled(boolean z4) {
        if (this.mCircleMaskEnabled != z4) {
            this.mCircleMaskEnabled = z4;
            ...
            public final void run() {
                ...
                synchronized (FrameSequenceDrawable.this.mLock) {
                    ...
                    try {
                        ...
                        new StringBuilder("exception during decode: ").append(e5);
                        z4 = true;
                    }
                    if (j4 < FrameSequenceDrawable.MIN_DELAY_MS) {
                        j4 = 100;
                    }
                    synchronized (FrameSequenceDrawable.this.mLock) {
                        bitmap = null;
                        if (FrameSequenceDrawable.this.mDestroyed) {
                            ...
                            frameSequenceDrawable.mNextSwap = z4 ? Long.MAX_VALUE : j4 + frameSequenceDrawable.mLastSwap;
