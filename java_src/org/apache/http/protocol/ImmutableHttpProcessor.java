...
public final class ImmutableHttpProcessor implements HttpProcessor {
    ...
    public ImmutableHttpProcessor(HttpRequestInterceptorList httpRequestInterceptorList, HttpResponseInterceptorList httpResponseInterceptorList) {
        if (httpRequestInterceptorList != null) {
            ...
            for (int i4 = 0; i4 < requestInterceptorCount; i4++) {
                this.requestInterceptors[i4] = httpRequestInterceptorList.getRequestInterceptor(i4);
            }
        } else {
            this.requestInterceptors = new HttpRequestInterceptor[0];
        }
        if (httpResponseInterceptorList != null) {
            ...
            for (int i5 = 0; i5 < responseInterceptorCount; i5++) {
                this.responseInterceptors[i5] = httpResponseInterceptorList.getResponseInterceptor(i5);
            }
            return;
        }
        this.responseInterceptors = new HttpResponseInterceptor[0];
