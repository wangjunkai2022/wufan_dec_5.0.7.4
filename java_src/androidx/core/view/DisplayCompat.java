        ...
        public static ModeCompat[] getSupportedModes(@NonNull Context context, @NonNull Display display) {
            ...
            if (currentDisplaySizeFromWorkarounds != null && !physicalSizeEquals(mode, currentDisplaySizeFromWorkarounds)) {
                for (int i4 = 0; i4 < supportedModes.length; i4++) {
                    if (physicalSizeEquals(supportedModes[i4], mode)) {
                        modeCompat = new ModeCompat(supportedModes[i4], currentDisplaySizeFromWorkarounds);
                    } else {
                        modeCompat = new ModeCompat(supportedModes[i4], false);
                    }
