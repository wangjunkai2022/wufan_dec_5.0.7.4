...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    public static class ClampedScroller {
        ...
        private float getValueAt(long j4) {
            ...
            if (j6 >= 0 && j4 >= j6) {
                ...
                return (1.0f - f5) + (f5 * AutoScrollHelper.constrain(((float) (j4 - j6)) / this.mEffectiveRampDown, 0.0f, 1.0f));
            }
    ...
    private float computeTargetVelocity(int i4, float f5, float f6, float f7) {
        float edgeValue = getEdgeValue(this.mRelativeEdges[i4], f6, this.mMaximumEdges[i4], f5);
        if (edgeValue == 0.0f) {
            return 0.0f;
        }
        float f8 = this.mRelativeVelocity[i4];
        float f9 = this.mMinimumVelocity[i4];
        float f10 = this.mMaximumVelocity[i4];
        float f11 = f8 * f7;
        if (edgeValue > 0.0f) {
