...
public final class ColorStateListInflaterCompat {
    ...
    @ColorInt
    private static int modulateColorAlpha(@ColorInt int i4, @FloatRange(from = 0.0d, to = 1.0d) float f5, @FloatRange(from = 0.0d, to = 100.0d) float f6) {
        boolean z4 = f6 >= 0.0f && f6 <= 100.0f;
        if (f5 != 1.0f || z4) {
            int clamp = MathUtils.clamp((int) ((Color.alpha(i4) * f5) + 0.5f), 0, 255);
            if (z4) {
                CamColor fromColor = CamColor.fromColor(i4);
                i4 = CamColor.toColor(fromColor.getHue(), fromColor.getChroma(), f6);
            }
            return (i4 & 16777215) | (clamp << 24);
        }
        return i4;
    }
