...
public final class HttpHeaders {
    private static final ByteString QUOTED_STRING_DELIMITERS = ByteString.encodeUtf8(BasicHeaderValueFormatterHC4.UNSAFE_CHARS);
    ...
    public static boolean hasBody(Response response) {
        ...
        return (((code >= 100 && code < 200) || code == 204 || code == 304) && contentLength(response) == -1 && !HttpHeaders.Values.CHUNKED.equalsIgnoreCase(response.header("Transfer-Encoding"))) ? false : true;
    }
    ...
    */
    ...
    private static String repeat(char c5, int i4) {
        char[] cArr = new char[i4];
        Arrays.fill(cArr, c5);
        return new String(cArr);
    }

    private static int skipAll(Buffer buffer, byte b5) {
        int i4 = 0;
