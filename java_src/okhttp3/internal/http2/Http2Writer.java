...
final class Http2Writer implements Closeable {
    ...
    public synchronized void data(boolean z4, int i4, Buffer buffer, int i5) throws IOException {
        if (!this.closed) {
            dataFrame(i4, z4 ? (byte) 1 : (byte) 0, buffer, i5);
        } else {
            throw new IOException("closed");
        }
    }

    void dataFrame(int i4, byte b5, Buffer buffer, int i5) throws IOException {
    ...
    void headers(boolean z4, int i4, List<Header> list) throws IOException {
        if (!this.closed) {
            this.hpackWriter.writeHeaders(list);
            long size = this.hpackBuffer.size();
