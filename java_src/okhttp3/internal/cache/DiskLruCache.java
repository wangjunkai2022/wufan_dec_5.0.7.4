...
public final class DiskLruCache implements Closeable, Flushable {
        ...
        public Sink newSink(int i4) {
            synchronized (DiskLruCache.this) {
                if (!this.done) {
                    ...
                    try {
                        return new FaultHidingSink(DiskLruCache.this.fileSystem.sink(entry.dirtyFiles[i4])) { // from class: okhttp3.internal.cache.DiskLruCache.Editor.1
    ...
    private void validateKey(String str) {
        ...
        throw new IllegalArgumentException("keys must match regex [a-z0-9_-]{1,120}: \"" + str + a.f71493g);
    }
    ...
    synchronized void completeEdit(Editor editor, boolean z4) throws IOException {
        Entry entry = editor.entry;
        if (entry.currentEditor == editor) {
            if (z4 && !entry.readable) {
