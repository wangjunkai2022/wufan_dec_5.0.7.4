...
public abstract class Maybe<T> implements MaybeSource<T> {
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public static <T> Maybe<T> just(T t4) {
        ObjectHelper.requireNonNull(t4, "item is null");
        return RxJavaPlugins.onAssembly(new MaybeJust(t4));
    }
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public final Maybe<T> defaultIfEmpty(T t4) {
        ObjectHelper.requireNonNull(t4, "item is null");
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public final Maybe<T> onErrorReturnItem(T t4) {
        ObjectHelper.requireNonNull(t4, "item is null");
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public final <E extends MaybeObserver<? super T>> E subscribeWith(E e5) {
        subscribe(e5);
        return e5;
    }
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public final Single<T> toSingle(T t4) {
        ObjectHelper.requireNonNull(t4, "defaultValue is null");
        return RxJavaPlugins.onAssembly(new MaybeToSingle(this, t4));
    }
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public static <T, D> Maybe<T> using(Callable<? extends D> callable, Function<? super D, ? extends MaybeSource<? extends T>> function, Consumer<? super D> consumer, boolean z4) {
        ObjectHelper.requireNonNull(callable, "resourceSupplier is null");
        ObjectHelper.requireNonNull(function, "sourceSupplier is null");
        ObjectHelper.requireNonNull(consumer, "disposer is null");
        return RxJavaPlugins.onAssembly(new MaybeUsing(callable, function, consumer, z4));
    }
    ...
    public static <T> Flowable<T> merge(Publisher<? extends MaybeSource<? extends T>> publisher, int i4) {
        ...
        return RxJavaPlugins.onAssembly(new FlowableFlatMapPublisher(publisher, MaybeToPublisher.instance(), false, i4, 1));
    }

    @SchedulerSupport("none")
    @BackpressureSupport(BackpressureKind.FULL)
    @CheckReturnValue
    public static <T> Flowable<T> mergeDelayError(Publisher<? extends MaybeSource<? extends T>> publisher, int i4) {
        ObjectHelper.requireNonNull(publisher, "source is null");
        ObjectHelper.verifyPositive(i4, "maxConcurrency");
        return RxJavaPlugins.onAssembly(new FlowableFlatMapPublisher(publisher, MaybeToPublisher.instance(), true, i4, 1));
    }
    ...
    public final T blockingGet(T t4) {
        ...
        BlockingMultiObserver blockingMultiObserver = new BlockingMultiObserver();
        subscribe(blockingMultiObserver);
        return (T) blockingMultiObserver.blockingGet(t4);
    }
