...
public final class HalfSerializer {
    ...
    public static <T> void onNext(Subscriber<? super T> subscriber, T t4, AtomicInteger atomicInteger, AtomicThrowable atomicThrowable) {
    ...
    /* JADX WARN: Multi-variable type inference failed */
    public static <T> void onNext(Observer<? super T> observer, T t4, AtomicInteger atomicInteger, AtomicThrowable atomicThrowable) {
        if (atomicInteger.get() == 0 && atomicInteger.compareAndSet(0, 1)) {
            observer.onNext(t4);
            if (atomicInteger.decrementAndGet() != 0) {
                Throwable terminate = atomicThrowable.terminate();
