...
public final class QueueDrainHelper {
    ...
    public static <T, U> boolean checkTerminated(boolean z4, boolean z5, Subscriber<?> subscriber, boolean z6, SimpleQueue<?> simpleQueue, QueueDrain<T, U> queueDrain) {
    ...
    public static <T, U> void drainLoop(SimplePlainQueue<T> simplePlainQueue, Observer<? super U> observer, boolean z4, Disposable disposable, ObservableQueueDrain<T, U> observableQueueDrain) {
        int i4 = 1;
        while (!checkTerminated(observableQueueDrain.done(), simplePlainQueue.isEmpty(), observer, z4, simplePlainQueue, disposable, observableQueueDrain)) {
            while (true) {
                boolean done = observableQueueDrain.done();
                T poll = simplePlainQueue.poll();
                boolean z5 = poll == null;
                if (checkTerminated(done, z5, observer, z4, simplePlainQueue, disposable, observableQueueDrain)) {
                    return;
                }
                if (z5) {
    ...
    public static <T, U> void drainMaxLoop(SimplePlainQueue<T> simplePlainQueue, Subscriber<? super U> subscriber, boolean z4, Disposable disposable, QueueDrain<T, U> queueDrain) {
        int i4 = 1;
        while (true) {
            boolean done = queueDrain.done();
            T poll = simplePlainQueue.poll();
            boolean z5 = poll == null;
            if (checkTerminated(done, z5, subscriber, z4, simplePlainQueue, queueDrain)) {
                if (disposable != null) {
                    disposable.dispose();
                    return;
                }
    ...
    public static <T, U> boolean checkTerminated(boolean z4, boolean z5, Observer<?> observer, boolean z6, SimpleQueue<?> simpleQueue, Disposable disposable, ObservableQueueDrain<T, U> observableQueueDrain) {
