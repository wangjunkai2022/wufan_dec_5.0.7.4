...
public abstract class Completable implements CompletableSource {
    ...
    @CheckReturnValue
    private static Completable merge0(Publisher<? extends CompletableSource> publisher, int i4, boolean z4) {
        ObjectHelper.requireNonNull(publisher, "sources is null");
        ObjectHelper.verifyPositive(i4, "maxConcurrency");
        return RxJavaPlugins.onAssembly(new CompletableMerge(publisher, i4, z4));
    }
    ...
    public final <T> Single<T> toSingleDefault(T t4) {
        ObjectHelper.requireNonNull(t4, "completionValue is null");
        return RxJavaPlugins.onAssembly(new CompletableToSingle(this, null, t4));
    }
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public static <R> Completable using(Callable<R> callable, Function<? super R, ? extends CompletableSource> function, Consumer<? super R> consumer, boolean z4) {
        ObjectHelper.requireNonNull(callable, "resourceSupplier is null");
        ObjectHelper.requireNonNull(function, "completableFunction is null");
        ObjectHelper.requireNonNull(consumer, "disposer is null");
        return RxJavaPlugins.onAssembly(new CompletableUsing(callable, function, consumer, z4));
    }
    ...
    public final Completable delay(long j4, TimeUnit timeUnit, Scheduler scheduler, boolean z4) {
        ObjectHelper.requireNonNull(timeUnit, "unit is null");
        ObjectHelper.requireNonNull(scheduler, "scheduler is null");
        return RxJavaPlugins.onAssembly(new CompletableDelay(this, j4, timeUnit, scheduler, z4));
    }
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public final TestObserver<Void> test(boolean z4) {
        TestObserver<Void> testObserver = new TestObserver<>();
        if (z4) {
            testObserver.cancel();
        }
    ...
    @SchedulerSupport("none")
    @CheckReturnValue
    public final Completable retry(long j4, Predicate<? super Throwable> predicate) {
        return fromPublisher(toFlowable().retry(j4, predicate));
    }
