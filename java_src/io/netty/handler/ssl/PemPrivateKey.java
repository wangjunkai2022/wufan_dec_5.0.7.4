...
public final class PemPrivateKey extends AbstractReferenceCounted implements PrivateKey, PemEncoded {
    ...
    public static PemEncoded toPEM(ByteBufAllocator byteBufAllocator, boolean z4, PrivateKey privateKey) {
        if (privateKey instanceof PemEncoded) {
            return ((PemEncoded) privateKey).retain();
        }
        byte[] encoded = privateKey.getEncoded();
        if (encoded != null) {
            return toPEM(byteBufAllocator, z4, encoded);
        }
    ...
    static PemEncoded toPEM(ByteBufAllocator byteBufAllocator, boolean z4, byte[] bArr) {
        ...
        try {
            ...
            ByteBuf directBuffer = z4 ? byteBufAllocator.directBuffer(length2) : byteBufAllocator.buffer(length2);
