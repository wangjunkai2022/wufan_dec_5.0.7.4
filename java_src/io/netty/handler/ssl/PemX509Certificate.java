...
public final class PemX509Certificate extends X509Certificate implements PemEncoded {
    ...
    private static ByteBuf append(ByteBufAllocator byteBufAllocator, boolean z4, PemEncoded pemEncoded, int i4, ByteBuf byteBuf) {
        ByteBuf content = pemEncoded.content();
        if (byteBuf == null) {
            byteBuf = newBuffer(byteBufAllocator, z4, content.readableBytes() * i4);
        }
        byteBuf.writeBytes(content.slice());
        return byteBuf;
    }

    private static ByteBuf newBuffer(ByteBufAllocator byteBufAllocator, boolean z4, int i4) {
        return z4 ? byteBufAllocator.directBuffer(i4) : byteBufAllocator.buffer(i4);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static PemEncoded toPEM(ByteBufAllocator byteBufAllocator, boolean z4, X509Certificate... x509CertificateArr) throws CertificateEncodingException {
                ...
                for (X509Certificate x509Certificate2 : x509CertificateArr) {
                    if (x509Certificate2 != null) {
                        if (x509Certificate2 instanceof PemEncoded) {
                            byteBuf = append(byteBufAllocator, z4, (PemEncoded) x509Certificate2, x509CertificateArr.length, byteBuf);
                        } else {
                            byteBuf = append(byteBufAllocator, z4, x509Certificate2, x509CertificateArr.length, byteBuf);
                        }
                    } else {
    ...
    private static ByteBuf append(ByteBufAllocator byteBufAllocator, boolean z4, X509Certificate x509Certificate, int i4, ByteBuf byteBuf) throws CertificateEncodingException {
        ...
        try {
            ...
            if (byteBuf == null) {
                byteBuf = newBuffer(byteBufAllocator, z4, (BEGIN_CERT.length + base64.readableBytes() + END_CERT.length) * i4);
            }
