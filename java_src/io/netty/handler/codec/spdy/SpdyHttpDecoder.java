...
public class SpdyHttpDecoder extends MessageToMessageDecoder<SpdyFrame> {
    ...
    private static FullHttpResponse createHttpResponse(SpdyHeadersFrame spdyHeadersFrame, ByteBufAllocator byteBufAllocator, boolean z4) throws Exception {
        SpdyHeaders headers = spdyHeadersFrame.headers();
        ...
        ByteBuf buffer = byteBufAllocator.buffer();
        try {
            DefaultFullHttpResponse defaultFullHttpResponse = new DefaultFullHttpResponse(valueOf, parseLine, buffer, z4);
            for (Map.Entry<CharSequence, CharSequence> entry : spdyHeadersFrame.headers()) {
    ...
    public SpdyHttpDecoder(SpdyVersion spdyVersion, int i4, boolean z4) {
        this(spdyVersion, i4, new HashMap(), z4);
    }
    ...
    protected SpdyHttpDecoder(SpdyVersion spdyVersion, int i4, Map<Integer, FullHttpMessage> map, boolean z4) {
        this.spdyVersion = ((SpdyVersion) ObjectUtil.checkNotNull(spdyVersion, "version")).getVersion();
        this.maxContentLength = ObjectUtil.checkPositive(i4, "maxContentLength");
        this.messageMap = map;
