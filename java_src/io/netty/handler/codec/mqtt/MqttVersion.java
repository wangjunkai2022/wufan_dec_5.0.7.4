...
public enum MqttVersion {
    ...
    MqttVersion(String str, byte b5) {
        this.name = (String) ObjectUtil.checkNotNull(str, "protocolName");
        this.level = b5;
    }

    public static MqttVersion fromProtocolNameAndLevel(String str, byte b5) {
        MqttVersion[] values;
        for (MqttVersion mqttVersion : values()) {
            if (mqttVersion.name.equals(str)) {
                if (mqttVersion.level == b5) {
                    return mqttVersion;
                }
                throw new MqttUnacceptableProtocolVersionException(str + " and " + ((int) b5) + " are not match");
            }
        }
