...
public class Http2ConnectionHandler extends ByteToMessageDecoder implements Http2LifecycleManager, ChannelOutboundHandler {
    ...
    public static final class ClosingChannelFutureListener implements ChannelFutureListener {
        ...
        ClosingChannelFutureListener(ChannelHandlerContext channelHandlerContext, ChannelPromise channelPromise, long j4, TimeUnit timeUnit) {
            this.ctx = channelHandlerContext;
            this.promise = channelPromise;
    ...
    private ChannelFutureListener newClosingChannelFutureListener(ChannelHandlerContext channelHandlerContext, ChannelPromise channelPromise) {
        long j4 = this.gracefulShutdownTimeoutMillis;
        return j4 < 0 ? new ClosingChannelFutureListener(channelHandlerContext, channelPromise) : new ClosingChannelFutureListener(channelHandlerContext, channelPromise, j4, TimeUnit.MILLISECONDS);
    }
    ...
    private ChannelFuture resetUnknownStream(final ChannelHandlerContext channelHandlerContext, int i4, long j4, ChannelPromise channelPromise) {
        ChannelFuture writeRstStream = frameWriter().writeRstStream(channelHandlerContext, i4, j4, channelPromise);
    ...
    public void closeStreamLocal(Http2Stream http2Stream, ChannelFuture channelFuture) {
        int i4 = AnonymousClass6.$SwitchMap$io$netty$handler$codec$http2$Http2Stream$State[http2Stream.state().ordinal()];
        if (i4 != 1 && i4 != 2) {
            closeStream(http2Stream, channelFuture);
        } else {
    ...
    public void flush(ChannelHandlerContext channelHandlerContext) {
        try {
            ...
            onError(channelHandlerContext, true, e5);
        } catch (Throwable th) {
    ...
    public ChannelFuture goAway(final ChannelHandlerContext channelHandlerContext, final int i4, final long j4, final ByteBuf byteBuf, ChannelPromise channelPromise) {
        ChannelPromise unvoid = channelPromise.unvoid();
        try {
            if (!connection().goAwaySent(i4, j4, byteBuf)) {
                byteBuf.release();
                unvoid.trySuccess();
                return unvoid;
            }
            byteBuf.retain();
            ChannelFuture writeGoAway = frameWriter().writeGoAway(channelHandlerContext, i4, j4, byteBuf, unvoid);
    ...
    @Override // io.netty.handler.codec.http2.Http2LifecycleManager
    public void onError(ChannelHandlerContext channelHandlerContext, boolean z4, Throwable th) {
        ...
        if (Http2Exception.isStreamError(embeddedHttp2Exception)) {
            onStreamError(channelHandlerContext, z4, th, (Http2Exception.StreamException) embeddedHttp2Exception);
        } else if (embeddedHttp2Exception instanceof Http2Exception.CompositeStreamException) {
            Iterator<Http2Exception.StreamException> it2 = ((Http2Exception.CompositeStreamException) embeddedHttp2Exception).iterator();
            while (it2.hasNext()) {
                onStreamError(channelHandlerContext, z4, th, it2.next());
            }
        } else {
            onConnectionError(channelHandlerContext, z4, th, embeddedHttp2Exception);
        }
        channelHandlerContext.flush();
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void onStreamError(ChannelHandlerContext channelHandlerContext, boolean z4, Throwable th, Http2Exception.StreamException streamException) {
        ...
        if ((streamException instanceof Http2Exception.HeaderListSizeException) && ((Http2Exception.HeaderListSizeException) streamException).duringDecode() && connection().isServer()) {
            ...
            if (stream != null && !stream.isHeadersSent()) {
                try {
                    ...
                    onError(channelHandlerContext, z4, Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, th2, "Error DecodeSizeError", new Object[0]));
                }
            }
        }
        Http2Stream http2Stream = stream;
        if (http2Stream == null) {
            if (!z4 || connection().local().mayHaveCreatedStream(streamId)) {
                resetUnknownStream(channelHandlerContext, streamId, streamException.error().code(), channelHandlerContext.newPromise());
                return;
            }
            return;
    ...
    @Override // io.netty.handler.codec.http2.Http2LifecycleManager
    public ChannelFuture resetStream(ChannelHandlerContext channelHandlerContext, int i4, long j4, ChannelPromise channelPromise) {
        Http2Stream stream = connection().stream(i4);
        if (stream == null) {
            return resetUnknownStream(channelHandlerContext, i4, j4, channelPromise.unvoid());
        }
        return resetStream(channelHandlerContext, stream, j4, channelPromise);
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public Http2ConnectionHandler(Http2ConnectionDecoder http2ConnectionDecoder, Http2ConnectionEncoder http2ConnectionEncoder, Http2Settings http2Settings, boolean z4) {
        ...
        this.decoupleCloseAndGoAway = z4;
    ...
    public void gracefulShutdownTimeoutMillis(long j4) {
        ...
        throw new IllegalArgumentException("gracefulShutdownTimeoutMillis: " + j4 + " (expected: -1 for indefinite or >= 0)");
    }

    private ChannelFuture resetStream(final ChannelHandlerContext channelHandlerContext, final Http2Stream http2Stream, long j4, ChannelPromise channelPromise) {
        ...
        if (http2Stream.state() != Http2Stream.State.IDLE && (!connection().local().created(http2Stream) || http2Stream.isHeadersSent() || http2Stream.isPushPromiseSent())) {
            success = frameWriter().writeRstStream(channelHandlerContext, http2Stream.id(), j4, unvoid);
        } else {
