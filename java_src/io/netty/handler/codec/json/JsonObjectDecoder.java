...
public class JsonObjectDecoder extends ByteToMessageDecoder {
    ...
    private void decodeByte(byte b5, ByteBuf byteBuf, int i4) {
        if ((b5 == 123 || b5 == 91) && !this.insideString) {
            this.openBraces++;
        } else if ((b5 == 125 || b5 == 93) && !this.insideString) {
            this.openBraces--;
        } else if (b5 == 34) {
    ...
    private void initDecoding(byte b5) {
        this.openBraces = 1;
        if (b5 == 91 && this.streamArrayElements) {
            this.state = 2;
        } else {
            this.state = 1;
        }
        ...
        if (writerIndex <= this.maxObjectLength) {
            while (i5 < writerIndex) {
                ...
                if (i6 == 1) {
                    ...
                    decodeByte(b5, byteBuf, i5);
                    if (!this.insideString && (((i4 = this.openBraces) == 1 && b5 == 44) || (i4 == 0 && b5 == 93))) {
    ...
    public JsonObjectDecoder(boolean z4) {
        this(1048576, z4);
    }

    public JsonObjectDecoder(int i4, boolean z4) {
        if (i4 >= 1) {
            this.maxObjectLength = i4;
            this.streamArrayElements = z4;
            return;
        }
