...
public class GlobalChannelTrafficShapingHandler extends AbstractTrafficShapingHandler {
    ...
    public GlobalChannelTrafficShapingHandler(ScheduledExecutorService scheduledExecutorService, long j4, long j5, long j6, long j7, long j8, long j9) {
        ...
        this.maxGlobalWriteSize = StatFsHelper.f10900i;
        createGlobalTrafficCounter(scheduledExecutorService);
        this.writeChannelLimit = j6;
        this.readChannelLimit = j7;
    }

    private long computeBalancedWait(float f5, float f6, long j4) {
        float f7;
        if (f6 == 0.0f) {
            return j4;
        }
        float f8 = f5 / f6;
        if (f8 > this.maxDeviation) {
            if (f8 < 1.0f - this.maxDeviation) {
                return j4;
            }
            f7 = this.slowDownFactor;
            if (j4 < 10) {
                j4 = 10;
            }
        } else {
            f7 = this.accelerationFactor;
        }
        return ((float) j4) * f7;
    }
    ...
    private void computeDeviationCumulativeBytes() {
        ...
        boolean z4 = false;
        boolean z5 = this.channelQueues.size() > 1;
        this.readDeviationActive = z5 && j7 < j6 / 2;
        if (z5 && j4 < j5 / 2) {
            z4 = true;
        }
        this.writeDeviationActive = z4;
    ...
    public void setMaxDeviation(float f5, float f6, float f7) {
        if (f5 > 0.4f) {
        ...
        if (f7 <= 0.0f) {
            this.maxDeviation = f5;
            this.accelerationFactor = f7 + 1.0f;
            this.slowDownFactor = f6 + 1.0f;
            return;
        }
    ...
    protected void submitWrite(final ChannelHandlerContext channelHandlerContext, Object obj, long j4, long j5, long j6, ChannelPromise channelPromise) {
        ...
        synchronized (perChannel2) {
            ...
            boolean z4 = this.queuesSize.get() > this.maxGlobalWriteSize;
            if (z4) {
