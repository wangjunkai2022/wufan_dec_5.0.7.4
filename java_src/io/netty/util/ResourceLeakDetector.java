...
public class ResourceLeakDetector<T> {
    ...
    public static final class DefaultResourceLeak<T> extends WeakReference<Object> implements ResourceLeakTracker<T>, ResourceLeak {
            ...
            if (ResourceLeakDetector.TARGET_RECORDS > 0) {
                do {
                    ...
                    if (i4 >= ResourceLeakDetector.TARGET_RECORDS) {
                        boolean z5 = PlatformDependent.threadLocalRandom().nextInt(1 << Math.min(i4 - ResourceLeakDetector.TARGET_RECORDS, 30)) != 0;
        ...
        public boolean close(T t4) {
            try {
                return close();
            } finally {
                reachabilityFence0(t4);
            }
        }
    }
    ...
    static {
        ...
        if (SystemPropertyUtil.get("io.netty.noResourceLeakDetection") != null) {
            z4 = SystemPropertyUtil.getBoolean("io.netty.noResourceLeakDetection", false);
            internalLoggerFactory.debug("-Dio.netty.noResourceLeakDetection: {}", Boolean.valueOf(z4));
    ...
    public static void setEnabled(boolean z4) {
        setLevel(z4 ? Level.SIMPLE : Level.DISABLED);
    }

    public static void setLevel(Level level2) {
        level = (Level) ObjectUtil.checkNotNull(level2, SimulatorExitPlayActivity_.O);
    }

    private DefaultResourceLeak track0(T t4) {
        Level level2 = level;
        ...
        if (level2.ordinal() < Level.PARANOID.ordinal()) {
            if (PlatformDependent.threadLocalRandom().nextInt(this.samplingInterval) == 0) {
                reportLeak();
                return new DefaultResourceLeak(t4, this.refQueue, this.allLeaks);
            }
            return null;
        }
        reportLeak();
    ...
    @Deprecated
    public final ResourceLeak open(T t4) {
        return track0(t4);
    }
    ...
    public final ResourceLeakTracker<T> track(T t4) {
        return track0(t4);
    }
