...
public final class NativeLibraryLoader {
    ...
    static {
        ...
        boolean z4 = SystemPropertyUtil.getBoolean("io.netty.native.deleteLibAfterLoading", true);
        DELETE_NATIVE_LIB_AFTER_LOADING = z4;
        InternalLogger internalLogger2 = logger;
        internalLogger2.debug("-Dio.netty.native.deleteLibAfterLoading: {}", Boolean.valueOf(z4));
        boolean z5 = SystemPropertyUtil.getBoolean("io.netty.native.tryPatchShadedId", true);
        TRY_TO_PATCH_SHADED_ID = z5;
        internalLogger2.debug("-Dio.netty.native.tryPatchShadedId: {}", Boolean.valueOf(z5));
    }
    ...
    private static String calculatePackagePrefix() {
        ...
        String replace = "io!netty!util!internal!NativeLibraryLoader".replace('!', (char) d.f68897a);
    ...
    */
    ...
    private static void loadLibrary(ClassLoader classLoader, String str, boolean z4) {
        try {
            try {
                loadLibraryByHelper(tryToLoadClass(classLoader, NativeLibraryUtil.class), str, z4);
                classLoader = logger;
                ...
                NativeLibraryUtil.loadLibrary(str, z4);
                logger.debug("Successfully loaded the library {}", str);
            } catch (UnsatisfiedLinkError e6) {
                logger.debug("Unable to load the library '{}', trying other loading mechanism.", str, e6);
                NativeLibraryUtil.loadLibrary(str, z4);
                logger.debug("Successfully loaded the library {}", str);
            }
        } catch (UnsatisfiedLinkError e7) {
            ThrowableUtil.addSuppressed(e7, (Throwable) classLoader);
            throw e7;
        }
    }

    private static void loadLibraryByHelper(final Class<?> cls, final String str, final boolean z4) throws UnsatisfiedLinkError {
        Object doPrivileged = AccessController.doPrivileged(new PrivilegedAction<Object>() { // from class: io.netty.util.internal.NativeLibraryLoader.1
            ...
            public Object run() {
                try {
                    ...
                    return method.invoke(null, str, Boolean.valueOf(z4));
                } catch (Exception e5) {
                    return e5;
                }
            }
    ...
    static boolean patchShadedLibraryId(InputStream inputStream, OutputStream outputStream, String str, String str2) throws IOException {
        ...
        if (patchShadedLibraryId(byteArray, str, str2)) {
            ...
            z4 = str.endsWith(str3) ? patchShadedLibraryId(byteArray, str.substring(0, str.length() - str3.length()), str2) : false;
        }
                    ...
                    return (Class) AccessController.doPrivileged(new PrivilegedAction<Class<?>>() { // from class: io.netty.util.internal.NativeLibraryLoader.2
                        ...
                        public Class<?> run() {
                            try {
                                ...
                                throw new IllegalStateException("Define class failed!", e6);
                            }
                        }
                    });
                } catch (ClassNotFoundException e6) {
                    ThrowableUtil.addSuppressed(e6, e5);
                    throw e6;
                } catch (Error e7) {
                    ThrowableUtil.addSuppressed(e7, e5);
                    throw e7;
                } catch (RuntimeException e8) {
                    ThrowableUtil.addSuppressed(e8, e5);
                    throw e8;
                }
            }
            throw e5;
