...
public final class DefaultPriorityQueue<T extends PriorityQueueNode> extends AbstractQueue<T> implements PriorityQueue<T> {
    ...
    private void bubbleDown(int i4, T t4) {
        int i5 = this.size >>> 1;
        while (i4 < i5) {
            int i6 = (i4 << 1) + 1;
            T[] tArr = this.queue;
            T t5 = tArr[i6];
            int i7 = i6 + 1;
            if (i7 < this.size && this.comparator.compare(t5, tArr[i7]) > 0) {
                t5 = this.queue[i7];
                i6 = i7;
            }
    ...
    private void bubbleUp(int i4, T t4) {
        while (i4 > 0) {
            int i5 = (i4 - 1) >>> 1;
            T t5 = this.queue[i5];
            if (this.comparator.compare(t4, t5) >= 0) {
    ...
    public boolean containsTyped(T t4) {
        return contains(t4, t4.priorityQueueIndex(this));
    }

    public boolean offer(T t4) {
        if (t4.priorityQueueIndex(this) == -1) {
            int i4 = this.size;
            T[] tArr = this.queue;
            if (i4 >= tArr.length) {
        ...
        throw new IllegalArgumentException("e.priorityQueueIndex(): " + t4.priorityQueueIndex(this) + " (expected: -1) + e: " + t4);
    }
    ...
    public void priorityChanged(T t4) {
        int priorityQueueIndex = t4.priorityQueueIndex(this);
        if (contains(t4, priorityQueueIndex)) {
            if (priorityQueueIndex == 0) {
                bubbleDown(priorityQueueIndex, t4);
                return;
            }
            if (this.comparator.compare(t4, this.queue[(priorityQueueIndex - 1) >>> 1]) < 0) {
    ...
    public boolean removeTyped(T t4) {
        int priorityQueueIndex = t4.priorityQueueIndex(this);
        if (contains(t4, priorityQueueIndex)) {
            t4.priorityQueueIndex(this, -1);
            int i4 = this.size - 1;
            ...
            if (i4 != 0 && i4 != priorityQueueIndex) {
                T[] tArr = this.queue;
                T t5 = tArr[i4];
                tArr[priorityQueueIndex] = t5;
                tArr[i4] = null;
                if (this.comparator.compare(t4, t5) < 0) {
                    bubbleDown(priorityQueueIndex, t5);
                } else {
                    bubbleUp(priorityQueueIndex, t5);
                }
                return true;
            }
