...
public abstract class DnsResolveContext<T> {
        ...
        public Iterator<InetSocketAddress> iterator() {
            return new Iterator<InetSocketAddress>() { // from class: io.netty.resolver.dns.DnsResolveContext.DnsAddressStreamList.1
                ...
                public boolean hasNext() {
                    return this.f69219i < this.stream.size();
                }
    ...
    static final class DnsResolveContextException extends RuntimeException {
        ...
        private DnsResolveContextException(String str, boolean z4) {
    ...
    private static String hostnameWithDot(String str) {
        if (StringUtil.endsWith(str, d.f68897a)) {
            return str;
        }
    ...
    */
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void query(final DnsServerAddressStream dnsServerAddressStream, final int i4, final DnsQuestion dnsQuestion, final DnsQueryLifecycleObserver dnsQueryLifecycleObserver, boolean z4, final Promise<List<T>> promise, Throwable th) {
        if (!this.completeEarly && i4 < dnsServerAddressStream.size() && this.allowedQueries != 0 && !promise.isCancelled()) {
            ...
            Future<AddressedEnvelope<DnsResponse, InetSocketAddress>> query0 = this.parent.query0(next, dnsQuestion, this.additionals, z4, newPromise, this.parent.ch.eventLoop().newPromise());
    ...
    abstract void cache(String str, DnsRecord[] dnsRecordArr, DnsRecord dnsRecord, T t4);
    ...
    abstract boolean isCompleteEarly(T t4);
    ...
    public void resolve(Promise<List<T>> promise) {
        ...
        if (searchDomains.length != 0 && this.parent.ndots() != 0 && !StringUtil.endsWith(this.hostname, d.f68897a)) {
            boolean hasNDots = hasNDots();
            if (hasNDots) {
                str = this.hostname;
            } else {
                str = this.hostname + d.f68897a + searchDomains[0];
            }
            int i4 = !hasNDots ? 1 : 0;
            Promise<List<T>> newPromise = this.parent.executor().newPromise();
    ...
    private boolean query(String str, DnsRecordType dnsRecordType, DnsServerAddressStream dnsServerAddressStream, boolean z4, Promise<List<T>> promise) {
        try {
            DefaultDnsQuestion defaultDnsQuestion = new DefaultDnsQuestion(str, dnsRecordType, this.dnsClass);
            query(dnsServerAddressStream, 0, defaultDnsQuestion, newDnsQueryLifecycleObserver(defaultDnsQuestion), z4, promise, null);
            return true;
        } catch (Throwable th) {
