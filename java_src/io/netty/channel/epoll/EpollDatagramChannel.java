...
public final class EpollDatagramChannel extends AbstractEpollChannel implements DatagramChannel {
            ...
            try {
                do {
                    ...
                    if (Native.IS_SUPPORTING_RECVMMSG) {
                        i4 = maxDatagramPayloadSize == 0 ? 1 : allocate.writableBytes() / maxDatagramPayloadSize;
                    } else {
                        i4 = 0;
                    }
                    if (i4 > 1) {
                        scatteringRead = EpollDatagramChannel.this.scatteringRead(recvBufAllocHandle, allocate, maxDatagramPayloadSize, i4);
                    } else if (isConnected) {
                        try {
    ...
    public boolean connectedRead(EpollRecvByteAllocatorHandle epollRecvByteAllocatorHandle, ByteBuf byteBuf, int i4) throws Exception {
        int writableBytes;
        int read;
        try {
            if (i4 != 0) {
                writableBytes = Math.min(byteBuf.writableBytes(), i4);
            } else {
    ...
    */
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public boolean read(EpollRecvByteAllocatorHandle epollRecvByteAllocatorHandle, ByteBuf byteBuf, int i4) throws IOException {
        int writableBytes;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public boolean scatteringRead(EpollRecvByteAllocatorHandle epollRecvByteAllocatorHandle, ByteBuf byteBuf, int i4, int i5) throws IOException {
        RecyclableArrayList recyclableArrayList = null;
        try {
            int writerIndex = byteBuf.writerIndex();
            NativeDatagramPacketArray cleanDatagramPacketArray = cleanDatagramPacketArray();
            int i6 = 0;
            while (i6 < i5 && cleanDatagramPacketArray.addWritable(byteBuf, writerIndex, i4)) {
                i6++;
                writerIndex += i4;
            }
            ...
            for (int i8 = 0; i8 < recvmmsg; i8++) {
                newInstance.add(packets[i8].newDatagramPacket(byteBuf.readRetainedSlice(i4), localAddress));
            }
            epollRecvByteAllocatorHandle.lastBytesRead(i7);
            epollRecvByteAllocatorHandle.incMessagesRead(recvmmsg);
            for (int i9 = 0; i9 < recvmmsg; i9++) {
                pipeline().fireChannelRead(newInstance.set(i9, Unpooled.EMPTY_BUFFER));
            }
            newInstance.recycle();
            byteBuf.release();
            return true;
        ...
        while (true) {
            ...
            if (Native.IS_SUPPORTING_SENDMMSG && channelOutboundBuffer.size() > 1) {
                ...
                if (count >= 1) {
                    ...
                    int i4 = 0;
                    while (count > 0) {
                        int sendmmsg = this.socket.sendmmsg(packets, i4, count);
                        if (sendmmsg == 0) {
    ...
    */
    ...
    public EpollDatagramChannel(int i4) {
        this(new LinuxSocket(i4), true);
    }
    ...
    private EpollDatagramChannel(LinuxSocket linuxSocket, boolean z4) {
