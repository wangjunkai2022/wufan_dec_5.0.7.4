...
public class NioUdtMessageConnectorChannel extends AbstractNioMessageChannel implements UdtChannel {
    ...
    protected void doBind(SocketAddress socketAddress) throws Exception {
        privilegedBind(mo30javaChannel(), socketAddress);
    }
    ...
    protected int doReadMessages(List<Object> list) throws Exception {
        ...
        int writeBytes = directBuffer.writeBytes((ScatteringByteChannel) mo30javaChannel(), receiveBufferSize);
        if (writeBytes <= 0) {
    ...
    protected boolean doWriteMessage(Object obj, ChannelOutboundBuffer channelOutboundBuffer) throws Exception {
        ...
        if (content.nioBufferCount() == 1) {
            write = mo30javaChannel().write(content.nioBuffer());
        } else {
            write = mo30javaChannel().write(content.nioBuffers());
        }
    ...
    public boolean isActive() {
        SocketChannelUDT mo30javaChannel = mo30javaChannel();
        return mo30javaChannel.isOpen() && mo30javaChannel.isConnectFinished();
    }
    ...
    public NioUdtMessageConnectorChannel(Channel channel, SocketChannelUDT socketChannelUDT) {
        ...
        try {
            ...
            int i4 = AnonymousClass2.$SwitchMap$com$barchart$udt$StatusUDT[socketChannelUDT.socketUDT().status().ordinal()];
            if (i4 != 1 && i4 != 2) {
    ...
    public SocketChannelUDT mo30javaChannel() {
