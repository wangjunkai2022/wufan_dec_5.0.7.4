...
public class NioSctpChannel extends AbstractNioMessageChannel implements SctpChannel {
    ...
    public Set<InetSocketAddress> allLocalAddresses() {
        try {
            Set<SocketAddress> allLocalAddresses = mo30javaChannel().getAllLocalAddresses();
    ...
    public Set<InetSocketAddress> allRemoteAddresses() {
        try {
            Set<SocketAddress> remoteAddresses = mo30javaChannel().getRemoteAddresses();
    ...
    public Association association() {
        try {
            return mo30javaChannel().association();
    ...
    protected void doBind(SocketAddress socketAddress) throws Exception {
        mo30javaChannel().bind(socketAddress);
    }
    ...
    protected boolean doConnect(SocketAddress socketAddress, SocketAddress socketAddress2) throws Exception {
        if (socketAddress2 != null) {
            mo30javaChannel().bind(socketAddress2);
        }
        try {
            boolean connect = mo30javaChannel().connect(socketAddress);
            if (!connect) {
    ...
    protected int doReadMessages(List<Object> list) throws Exception {
        com.sun.nio.sctp.SctpChannel mo30javaChannel = mo30javaChannel();
        ...
        try {
            ...
            MessageInfo receive = mo30javaChannel.receive(internalNioBuffer, (Object) null, this.notificationHandler);
            if (receive == null) {
                return 0;
            }
    ...
    protected boolean doWriteMessage(Object obj, ChannelOutboundBuffer channelOutboundBuffer) throws Exception {
        ...
        boolean z4 = content.nioBufferCount() != 1;
        if (!z4 && !content.isDirect() && alloc.isDirectBufferPooled()) {
            z4 = true;
        }
        if (z4) {
        ...
        return mo30javaChannel().send(nioBuffer, createOutgoing) > 0;
    }
    ...
    public boolean isActive() {
        return mo30javaChannel().isOpen() && association() != null;
    }

    @Override // io.netty.channel.AbstractChannel
    protected SocketAddress localAddress0() {
        try {
            Iterator it2 = mo30javaChannel().getAllLocalAddresses().iterator();
            if (it2.hasNext()) {
    ...
    protected SocketAddress remoteAddress0() {
        try {
            Iterator it2 = mo30javaChannel().getRemoteAddresses().iterator();
            if (it2.hasNext()) {
                return (SocketAddress) it2.next();
            }
            return null;
    ...
    public ChannelFuture bindAddress(final InetAddress inetAddress, final ChannelPromise channelPromise) {
        if (eventLoop().inEventLoop()) {
            try {
                mo30javaChannel().bindAddress(inetAddress);
    ...
    public com.sun.nio.sctp.SctpChannel mo30javaChannel() {
    ...
    public ChannelFuture unbindAddress(final InetAddress inetAddress, final ChannelPromise channelPromise) {
        if (eventLoop().inEventLoop()) {
            try {
                mo30javaChannel().unbindAddress(inetAddress);
                channelPromise.setSuccess();
            } catch (Throwable th) {
                channelPromise.setFailure(th);
            }
        ...
        try {
            ...
            try {
                ...
                if (logger.isWarnEnabled()) {
                    logger.warn("Failed to close a partially initialized sctp channel.", (Throwable) e6);
                }
            }
